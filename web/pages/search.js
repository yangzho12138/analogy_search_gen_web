...
const SearchPage = ({
  ...
  const { preSetPrompt, preSetTemp, preSetQuery } = router.query;

  console.log(
    preSetPrompt,
    preSetTemp,
    preSetQuery,
    prompts.find((prompt) => prompt.includes(preSetPrompt))
  );
  const [query, setQuery] = useState(
    preSetQuery === undefined || preSetQuery == "" ? "" : preSetQuery
  );
    ...
    <div style={{ marginTop: "3%" }}>
      ...
      <Row>
        <Col md={3}>
          <img
            src="/images/logo.png"
            style={{ maxHeight: "15vh", width: "100%" }}
          />
        </Col>
        <Col md={6}>
        ...
        <Col md={3}>
          {userInfo === null ? (
            <>
              <Button variant="primary" disabled style={{ marginRight: "2%" }}>
                Assignment
              </Button>
              <Button variant="primary" disabled style={{ marginRight: "2%" }}>
                Generate
              ...
              <Link title="Subsribed user only, click to signin">
                <FontAwesomeIcon
      ...
      <Row>
        <Row style={{ marginLeft: "1%" }}>
          ...
          >
            New: Browse all analogies with images
          </div>
        </Row>
        <br />
        <Row style={{ marginLeft: "1%" }}>
                  ...
                  return (
                    <>
                      <AnalogyCard
                        ...
                        isCard={isCard}
                        userInfo={userInfo}
                        isCollected={collectedAnalogies.includes(result.pid)}
                        inCollection={false}
                      />
                      <br />
                    </>
...
SearchPage.getInitialProps = async ({ req }) => {
  ...
  try {
    ...
    if (res.status == 200) {
      ...
      collectedAnalogies = res.data.analogies.map((analogy) => analogy.pid);
    }
  } catch (err) {
    console.log(err);
  }
