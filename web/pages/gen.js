...
const link_title = {
  ...
  max_length:
    "The maximum number of tokens to generate shared between the prompt and the completion",
  top_p:
...
const GenPage = ({ userInfo }) => {
  ...
  const [max_length, setMaxLength] = useState(
    preSetMax_length === undefined ? 400 : parseInt(preSetMax_length)
  );
  ...
  if (userInfo) {
    ...
    return (
      <Row style={{ textAlign: "center", marginTop: "10%" }}>
        <h2>
          Only Subscribed User Could Use Generation Function,{" "}
